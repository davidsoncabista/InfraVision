...
- **Inteligência Artificial (Futuro):** Genkit (Google AI)
- **Versionamento:** Git
...
#### 4.2.3. Tabela de Evidências Genérica
**Status:** `Implementado`

Para unificar e simplificar o registro de eventos, a tabela `Evidence` foi refatorada. Em vez de estar ligada rigidamente a incidentes (`incidentId`), ela agora utiliza um sistema polimórfico com as colunas `entityType` (ex: 'Incidents', 'Connections') e `entityId`. Isso permite que qualquer entidade no sistema possa ter evidências associadas, abrindo caminho para futuros casos de uso, como anexar fotos a uma conexão de cabo ou documentar a instalação de um equipamento. A `Central de Evidências` também foi adaptada para registrar eventos gerais não associados a nenhuma entidade específica.

#### 4.2.4. Plataforma de Inteligência Assistida
**Status:** `Concluído`

Para acelerar drasticamente a entrada de dados e reduzir erros manuais, a plataforma de "Inteligência Assistida" foi desenvolvida em fases:

1.  **Motor de Análise (Texto e Imagem):**
    *   **Status:** ✅ `Concluído`
    *   **Conceito:** O motor de IA (`analyzeImportFlow`) foi construído para analisar e estruturar dados brutos, tanto de texto (CSV/colado) quanto de uma única imagem (usando OCR).
    *   **Inteligência de Conexão:** A IA foi instruída a extrair conexões De/Para. Caso o lado "Para" não seja identificado, a IA retorna o campo em branco, permitindo que o motor de backend crie uma "conexão unilateral" e um incidente de integridade automaticamente, refletindo um cenário de uso real.
    *   **Interface de Revisão:** Uma interface de revisão (`AnalysisReviewModal`) foi implementada para que o usuário possa editar e validar 100% dos dados analisados antes da importação final.

2.  **Lobby de Alocação:**
    *   **Status:** ✅ `Concluído`
    *   **Conceito:** Itens de planta baixa (como Racks) importados são criados "Em Depósito" (sem `roomId`). Na página `/datacenter`, um "lobby" lista esses itens. O usuário pode então arrastar e soltar os itens do lobby diretamente no mapa para alocá-los a uma sala e posição, garantindo a precisão espacial do inventário.

3.  **Captura Contínua por Vídeo (Scanner):**
    *   **Status:** ✅ `Concluído`
    *   **Conceito:** A funcionalidade final foi implementada, transformando a captura de imagem estática em um "scanner de vídeo" em tempo real. A aplicação agora captura e envia frames automaticamente para a IA, que analisa e agrega as informações.
    *   **Interface Interativa:** A interface do scanner exibe os resultados capturados em tempo real, permitindo ao usuário monitorar o progresso, selecionar a câmera desejada e decidir quando parar a captura para revisar os itens encontrados.
...
4. **11.4. Central de Evidências Unificada:**
    * **Status:** `Concluído`.
    * **Descrição:** A página `/evidence` foi criada para ser o ponto central de registro de dados operacionais. Ela permite o registro de "Evidências Gerais" não atreladas a itens, e também exibe um histórico das últimas evidências associadas a incidentes, unificando a visão sobre a atividade recente no sistema.
