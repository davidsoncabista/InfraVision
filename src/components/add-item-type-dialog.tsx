...
import { Loader2, Plus, Server, Snowflake, Router, Network, PanelTop, Database, Power, Fan, HardDrive, Box, Circle, Square } from "lucide-react";

import { addItemType } from "@/lib/item-types-actions";
...
export function AddItemTypeDialog({ isParentType, children }: AddItemTypeDialogProps) {
  ...
  const onSubmit = async (data: any) => {
    try {
      await addItemType(data, isParentType);
      toast({
        title: "Sucesso!",
      ...
      console.error("Falha ao adicionar tipo de item:", error);
      toast({
        variant: "destructive",
        title: "Erro",
        description: error.message || "Não foi possível adicionar o tipo de item.",
      });
    }
  };
              ...
              <FormControl>
                ...
                >
                  <FormItem className="flex items-center space-x-2 space-y-0">
                    <FormControl><RadioGroupItem value="rectangle" id="shape-rect" /></FormControl>
                    <FormLabel className="font-normal flex items-center gap-2" htmlFor="shape-rect"><Square className="h-4 w-4" /> Retângulo</FormLabel>
                  </FormItem>
                  <FormItem className="flex items-center space-x-2 space-y-0">
                    <FormControl><RadioGroupItem value="circle" id="shape-circle" /></FormControl>
                    <FormLabel className="font-normal flex items-center gap-2" htmlFor="shape-circle"><Circle className="h-4 w-4" /> Círculo</FormLabel>
                  </FormItem>
                </RadioGroup>
              </FormControl>
              <FormMessage />
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {isParentType 
              ? "Defina os atributos para um novo tipo de equipamento da planta baixa."
              : "Defina um novo tipo genérico de equipamento aninhado (Ex: Servidor)."
            }
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pt-4">
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Adicionando...</> : "Adicionar Tipo"}
              </Button>
            </DialogFooter>
          </form>
        </Form>
